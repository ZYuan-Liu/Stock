...
public class SmartViewHolder extends RecyclerView.ViewHolder implements View.OnClickListener {
    ...
    public SmartViewHolder text(int id, CharSequence sequence, String color, Context context) {
        View view = findViewById(id);

        int a = BaseActivity.getScreenWidth(context) / 4; //动态设置控件宽度
        LinearLayout.LayoutParams params = new LinearLayout.LayoutParams(a, LinearLayout.LayoutParams.WRAP_CONTENT);
        view.setLayoutParams(params);

        if (view instanceof TextView) {
            if (color == "green") {
                ((TextView) view).setText(sequence);
                ((TextView) view).setTextColor(context.getResources().getColor(R.color.stockgreen));

            } else if (color == "red") {
                ((TextView) view).setText(sequence);
                ((TextView) view).setTextColor(context.getResources().getColor(R.color.stockred));
            } else if (color == "gray") {
                ((TextView) view).setText(sequence);
                ((TextView) view).setTextColor(context.getResources().getColor(R.color.textgray1));
            } else {
                ((TextView) view).setText(sequence);
            }
        }
    ...
    public SmartViewHolder setimage(int id, String imageUrl, Context context) {
        View view = findViewById(id);
        if (view instanceof ImageView) {
            Picasso
                    .with(context)
